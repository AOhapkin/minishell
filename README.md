# minishell – это программа, которая обрабатывает команды и возвращает результат их выполнения. Ее главная цель — обеспечить взаимодействие между пользователем и ОС.
...
- [ ] Основная часть
  - [ ]  Простые команды и глобальные переменные
    - [x]  Выполнить `/bin/ls` или любую другую без аргументов
    - [x]  Выполнить пустую команду
    - [x]  Выполнить команду из пробелов табов
  - [ ]  Аргументы
  ...
  - [ ]  Возвращение значения процесса
    ...
    - [x]  Попробовать $? + $?
  - [ ] Сигналы
    - [x]  ctrl-C с пустым промптом должен сделать новую строку с новым промптом
    - [x]  ctrl-\ при пустом промпте ничего не делает
    - [x]  ctrl-D при пустом промпте должен сделать выход из minishell
    - [x]  ctrl-C, когда в промпте уже что-то написано, должен сделать новую строку с пустым промптом
    - [x]  ctrl-D, когда в промпте уже что-то написано, ничего не делает
    - [ ] ctrl-\, когда в промпте уже что-то написано,должен сделать выход из minishell
    - [x]  ctrl-C после выполнения блокирующей команды типа `cat` или `grep` без аргументов
    - [x] ctrl-\ после выполнения блокирующей команды типа `cat` или `grep` без аргументов
    - [x]  ctrl-D после выполнения блокирующей команды типа `cat` или `grep` без аргументов
  - [ ]  Двойные кавычки
  ...
  - [] Редиректы
    ...
    - [x]  Проверить, что << (не обновляет историю)
  - [ ]  Пайпы
    - [x]  Выполнить команды типа `cat file | greb bla | more`
    - [x]  Попробовать что-то невыполнимое типа `ls nrajehkfgewku | grep bla | more`
    - [x]  Попробовать смешать пайпы и редиректы
  - [ ]  Разные проверки
    - [x]  Набери команду, потом crtl-C, потом нажать enter. Буфер должен быть пустым и ничего не должно выполниться.
    ...
    - [x]  Попробуйте выполнить длинную команду с множеством аргументов
  - [ ]  Переменные окружения
    - [x]  Выполнить echo c любой переменной кружения
    - [x]  Проверить, что $ считается переменной окружения
    - [x]  Проверить, что двойные кавычки работают с $
